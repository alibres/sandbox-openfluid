...
#define BOOST_TEST_MODULE unittest_timeline
...
BOOST_AUTO_TEST_CASE(check_construction)
{
  ...
  std::list<openfluid::machine::ModelItemInstance*> fList;

  openfluid::machine::RunStepScheduler TL(fList,&SimStatus);

  BOOST_REQUIRE(!TL.hasTimePointToProcess());
}
...
{
  openfluid::base::SimulationStatus SimStatus(openfluid::core::DateTime(2012,1,1,0,0,0),openfluid::core::DateTime(2012,1,1,14,46,39),60);
  std::list<openfluid::machine::ModelItemInstance*> fList;

  openfluid::machine::ModelItemInstance* MII;

  MII = new openfluid::machine::ModelItemInstance();
  MII->Function = (openfluid::base::PluggableFunction*)(new FuncA());

  fList.push_back(MII);

  MII = new openfluid::machine::ModelItemInstance();
  MII->Function = (openfluid::base::PluggableFunction*)(new FuncB());

  fList.push_back(MII);

  MII = new openfluid::machine::ModelItemInstance();
  MII->Function = (openfluid::base::PluggableFunction*)(new FuncC());
  ...
  while (TL.hasTimePointToProcess())
  {
    std :: cout << " ******** " << SimStatus.getSeconds() << " ********" << std::endl;
    TL.processNextTimePoint();
    SimStatus.switchToUncheckedNextStep();
  }
